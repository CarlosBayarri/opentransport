...
def login(session, request):
    ...
    r=pg_operations2.pg_select2(d_conn, table_name='tabla.users', string_fields_to_select='typeuser,psw', cond_where='where usuario=%s', list_val_cond_where=[usuario])
...
def insertUsers():
    ...
    for u in lu:
        ...
        r=pg_operations2.pgInsert2(d_conn=d_conn, nom_tabla='tabla.users', d_str=d_str, str_fields_returning='gid')
        print r
